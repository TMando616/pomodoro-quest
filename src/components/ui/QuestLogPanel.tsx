...
export function QuestLogPanel({ logs, onClose }: QuestLogPanelProps) {
  ...
  const getDifficultyColor = (d: QuestLog['difficulty']) => {
    switch (d) {
      ...
      case 'hard': return 'text-orange-500 border-orange-500/20 bg-orange-500/5';
      case 'insane': return 'text-red-500 border-red-500/20 bg-red-500/5';
              ...
              <div key={log.id} className="bg-foreground/5 border-2 border-primary/10 rounded-2xl p-4 hover:border-primary/30 transition-all group">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex flex-col gap-1">
                    <h3 className="font-black text-primary transition-colors">
                      {log.name || "Untitled Quest"}
                    </h3>
                    <div className={`w-fit px-2 py-0.5 rounded border text-[7px] font-black uppercase tracking-tighter ${getDifficultyColor(log.difficulty)}`}>
                      {log.difficulty || 'normal'}
                    </div>
                  </div>
