...
import { Trophy, Crown, Zap, Coffee, Skull, Flame, Play, Pause, RotateCcw, BookOpen, MessageSquare, Target, CheckCircle2 } from 'lucide-react';
// 型定義と定数をインポート
  ...
  const handleComplete = useCallback((mode: 'quest' | 'rest', isBoss: boolean, duration: number, qName: string, difficulty: Difficulty) => {
    ...
    if (mode === 'quest') {
      ...
      if (currentUser) {
        ...
        // デイリーチャレンジ達成チェック (簡易版：今日の累計時間が目標を超えたか)
        ...
        <div className="w-full space-y-3">
          ...
          {guildInfo.globalMessage && (
            <div className="w-full bg-primary/10 border border-primary/20 rounded-2xl p-4 flex items-start gap-3 animate-in fade-in slide-in-from-top-4 duration-1000">
              ...
              <p className="text-[10px] font-bold leading-relaxed text-primary/80 italic">"{guildInfo.globalMessage}"</p>
            </div>
          )}
          ...
          <div className="w-full bg-foreground/5 border border-foreground/10 rounded-2xl p-4">
            ...
            {dqProgress >= 100 && (
              <div className="flex items-center gap-1 mt-2 text-yellow-500 animate-pulse">
                <CheckCircle2 className="w-3 h-3" />
                <span className="text-[8px] font-black uppercase tracking-tighter">Daily Objective Achieved</span>
              </div>
            )}
          </div>
        </div>
